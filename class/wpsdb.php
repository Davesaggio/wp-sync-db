...
class WPSDB extends WPSDB_Base {
	...
	public $checkbox_options; // Aggiunta della proprietà
...
return sprintf( '%s%s%s-%s-%s-%s.sql', $this->get_upload_info( $info_type ), $ds, sanitize_title_with_dashes( DB_NAME ),
$migration_type, $datetime, $this->session_salt );
}
...
$filtered_post = $this->filter_post_elements( $_POST, array( 'action', 'intent', 'url', 'key', 'form_data', 'prefix',
...
$preserved_options_data = $wpdb->get_results( sprintf( "SELECT * FROM %soptions WHERE `option_name` IN ('%s')",
$wpdb->prefix, implode( "','", $preserved_options ) ), ARRAY_A );
...
$filtered_post = $this->filter_post_elements( $_POST, array( 'action', 'intent', 'url', 'key', 'table', 'form_data',
...
// Does a quick check to make sure the verification string is valid and also opens /
// creates files for writing to (if required)
...
if ( false === $return ) {
$error_msg = __( 'Failed attempting to unserialize the response from the remote server. Please contact support.',
'wp-sync-db' );
...
// End point for the above remote_post call, ensures that the
// verification string is valid before continuing with the migration
...
$this->settings['profiles'][$profile]['select_post_types'] = array_values( array_diff( $response['post_types'],
...
$key = '';
for ( $i = 0; $i < 32; $i++ ) { $key .=substr( $keyset, rand( 0, strlen( $keyset ) -1 ), 1 ); } return $key; } function
    get_post_types() { global $wpdb; if( is_multisite() ) { $tables=$this->get_tables();
    ...
    // Debug: Registra un messaggio di errore se TABLE_NAME non è presente
    error_log('TABLE_NAME not set in results: ' . print_r($results, true));
    }
    }
    ...
    <div class="wrap wpsdb">
        ...
        <h2><?php _e( 'WP Sync DB', 'wp-sync-db' ); ?></h2>

        <h2 class="nav-tab-wrapper"><a href="#" class="nav-tab nav-tab-active js-action-link migrate"
                data-div-name="migrate-tab"><?php _e( 'Sync', 'wp-sync-db' ); ?></a><a href="#"
                class="nav-tab js-action-link settings"
                data-div-name="settings-tab"><?php _e( 'Settings', 'wp-sync-db' ); ?></a><a href="#"
                class="nav-tab js-action-link help" data-div-name="help-tab"><?php _e( 'Help', 'wp-sync-db' ); ?></a>
        </h2>
